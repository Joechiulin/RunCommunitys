...
class Communityfriend: UITableViewController {
   ...
   // let url_server = URL(string: "http://127.0.0.1:8080/ServerConnect_Web/ServerConnectServlet")
    ...
    @objc func showAllSpots() {
        ...
        var requestParam = [String: Any]()
            ...
            requestParam["action" ] = "getfriend"
            requestParam["account"] = account
        executeTask(url_server!, requestParam) { (datas, response, error) in
            if error == nil {
                if datas != nil {
                    // 將輸入資料列印出來除錯用
                    print("input: \(String(data: datas!, encoding: .utf8)!)")
                    
                    
                    
                    if let result = try? JSONDecoder().decode([Comm].self, from: datas!) {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cellId = "communityfriendcell"
        // tableViewCell預設的imageView點擊後會改變尺寸，所以建立UITableViewCell子類別SpotCell
        let cell = tableView.dequeueReusableCell(withIdentifier: cellId) as! Communityfriendcell
                ...
                if data != nil {
                    ...
                    if let result = try? JSONDecoder().decode([String:String].self, from: data!) {
                        DispatchQueue.main.async {
                            ...
                            cell.lbusernm.text = result["sqlname"]
                        }
                        /* 抓到資料後重刷table view */
        ...
        cell.inserttext.text = spot.text
        cell.lblik.text = like
